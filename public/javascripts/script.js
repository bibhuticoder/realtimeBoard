...
$(document).ready(function(){
    ...
    function insertText(event){
        ...
        tempDraw.push({txtData:$("#txtValue").val()});
        $("#txtValue").val("");
        mouseDown=true;
    ...
    function makeMouseDown(event){
        ...
        if(tool === 'text'){ 
            ...
            socket.emit('text',{text:tempDraw[0].txtData, x:iX, y:iY, color:currentColor, font:ctx.font })
            switchBoard('normal');
            changeTool('pencil');
        }
            ...
            //make eraser independent ot currentLineSize and currentColor
            var clickColor = ctx.getImageData(iX,iY,1,1);
            ...
            for(i=0; i<=width/10; i++){
                ...
                for(j=0; j<=height/10; j++){
                   ...
                   ctx.putImageData(clickColor,i,j);
                    
//                    if(ctx.getImageData(i,j,1,1).data === clickColor ){
//                        alert("match found");
//                    }
//                    
//                    else alert("not found");
    ...
    function makeMouseUp(){
        ...
        if(tool === 'pencil'){
            socket.emit("pencil", {boardData:tempDraw, brushSize:currentBrushSize, brushColor:currentColor});    
        }
        
        else if(tool === 'eraser'){
            
            socket.emit('eraser', {eraserData:tempDraw, eraserSize: currentEraserSize});
            clear('layer'); 
            switchBoard('normal');
            ...
            tempDraw.push({x:iX, y:iY});//store in array
            tempDraw.push({x:fX, y:fY});//''''
            socket.emit('line', {points:tempDraw, lineSize:currentLineSize, lineColor: currentColor});
            ...
            tempDraw.push({x:iX, y:iY, w:(fX-iX), h:(fY-iY)});//store in array
            socket.emit('rectangle', {points:tempDraw, lineSize:currentLineSize, lineColor: currentColor});
            ...
            socket.emit('ellipse', {points:{ix:iX, iy:iY, fx:fX, fy:fY, r:radius}, lineSize:currentLineSize, lineColor: currentColor});    
            ...
            tempDraw.push({selectedPart : ctx.getImageData(iX, iY, fX-iX, fY-iY), selected:true});
    ...
    function draw(e){
        ...
        if(tool === "pencil"){           
            ...
            ctx.fill();
            tempDraw.push({x:pos.x+8, y:pos.y+8});
    ...
    socket.on('ellipse',function(data){
        ...
        ctx.lineWidth = data.lineSize;
        ctx.beginPath();
        ctx.moveTo(data.points.ix,data.points.iy);                        ctx.bezierCurveTo(data.points.ix,data.points.fy,data.points.fx,data.points.fy,data.points.fx,data.points.iy);
